"""Command line interface for real-data acquisition.

The implementation lives here rather than in ``scripts/`` so that ``eee
download`` and ``python scripts/download_data.py`` are the same code path and
cannot drift apart. The script is kept as a thin wrapper for anyone who prefers
to run it directly without installing the package.
"""
...
from .download import config_from_environment, run_acquisition
from .sec_download import SecConfigurationError
